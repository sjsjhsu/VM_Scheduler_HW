...
# 读取虚拟机的 CPU 利用率数据
def load_vm_util(vm_id, index):
    vm_file = f"Hotspot/Hotspot/{vm_id}"  # 假设虚拟机数据文件存储在该路径
    try:
        return load_json(vm_file)["vm_util"][index]
...
def analyze_pairs(file):
    ...
    for pair in diff_data:
        ...
        vm2 = pair["vm2"]
        # index2 = pair["index2"]
        vm1_util = load_vm_util(vm1, 0)
        vm2_util = load_vm_util(vm2, 0)
...
def main():
    ...
    output_file = "json/analyzed_stats_results2.json"
    with open(output_file, 'w') as f:
        json.dump(results, f, indent=4)
